...
class CRAGPipeline:
    ...
    def _correct_path(self, query: str, best_doc: dict, scores: list) -> dict:
        ...
        sources = [self._format_source(best_doc["metadata"])]
        ref_str = f"[1] {best_doc['text']}"
        answer = self.generator(query=query, references=ref_str)
        return {"answer": answer, "sources": sources, "action": "correct", "scores": scores}
    ...
    def _ambiguous_path(self, query: str, best_doc: dict, scores: list) -> dict:
        ...
        all_sources = [self._format_source(best_doc["metadata"])] + web_sources
        ...
        answer = self.generator(query=query, references=ref_str)
        return {"answer": answer, "sources": all_sources, "action": "ambiguous", "scores": scores}
    ...
    def _web_search_path(self, query: str, scores: list = None, action: str = "web_only") -> dict:
        ...
        answer = self.generator(query=query, references=ref_str)
        return {"answer": answer, "sources": web_sources, "action": action, "scores": scores or []}
    ...
    def _format_source(metadata: dict) -> str:
        """Format metadata dict into a readable source string."""
